...
- **Every backend must be interactively logged in BEFORE the loop runs** — headless invocations only reuse stored credentials, they cannot complete OAuth. Observed live: unauthenticated `kiro-cli chat --no-interactive` ignores the flag and spins on "Opening browser..." until the iteration timeout kills it (exit 124), burning a full iteration per attempt. Add a login check to setup: invoke each configured backend once with a trivial prompt and a short timeout before starting L1.
- Nesting note (running vloop FROM another agent): Claude Code runs backends as plain child processes — works, subject only to its permission prompts. Codex sandboxes shell commands — child backends need network, so `codex exec` driving vloop needs `-s workspace-write` with network enabled (`-c sandbox_workspace_write.network_access=true`) or the loop's backends can't reach their APIs; prefer Mode B in a real terminal when in doubt.
