...
Breakers beyond iteration caps: liveness watchdog (`idle_timeout_s` no-output kill), hard run wall-clock budget (`max_wall_hours`), review-stalemate detection (`review_patience` identical judge findings = deadlock). Gates support `"baseline": true` delta mode for dirty repos (pre-existing failures waived, only NEW failure signatures block; empty-signature failures always block).
